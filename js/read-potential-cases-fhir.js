...
(async () => {
  ...
  await fs.writeFile("respiratory-failure-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("respiratory-failure-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
